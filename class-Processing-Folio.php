...
class ProcessingFolio {
	...
	private function __construct() {
		...
		add_action( 'init', array( $this, 'load_plugin_textdomain' ) );
		 add_shortcode("processing",  array( $this, 'processing_shortcode' ));
	...
	public function processing_shortcode( $name, $content="" ) {
		...
		$arrFilename = explode(' ', $content);
		/* Open the content - if it can't open, show thumbnail and an alternative download of the source code */
		$urls = '';
		foreach($arrFilename as $f) {
			$urls = $urls.$path.'/wp-content/uploads/'.$f.' ';
		}
		$result = '<div>';
		$result .= '<canvas datasrc="'.$urls.'">';
		$result .= '<p><b>Unfortunately your browser is not able to show this content.</b></p>';
		/* If the Post has a thumbnail, show it */
		if(has_post_thumbnail()){
			$result .= get_the_post_thumbnail();
		}
		$result .= '<p>You can download <a href="'.$path.'/wp-content/uploads/'.$content.'">'.$content.'</a> (Sourcecode) and compile it with <a href="http://processing.org/download/">Processing</a>.</p></canvas>';
		$result .= '</div>';		
		return $result;
	}	
